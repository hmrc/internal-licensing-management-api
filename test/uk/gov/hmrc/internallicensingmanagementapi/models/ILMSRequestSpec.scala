...
class ILMSRequestSpec extends BaseJsonFormattersSpec {
  ...
  "given a ILMSRequest" should {
    "produce json" in {
      Json.toJson(example) shouldBe Json.parse(Source.fromResource("./ilms-request-valid.json").mkString)
    ...
    "read json" in {
      ...
      testFromJson[ILMSRequest](json)(example)
    }
  }
}
